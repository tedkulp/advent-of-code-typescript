...
test("test solution 1", () => {
  expect(sol1.part1()).toBe(288);
});

test("calcs distances correctly for 7 mm", () => {
  expect([0, 1, 2, 3, 4, 5, 6, 7].map(n => {
    return sol1.calculateDistance(n, 7);
  })).toEqual([0, 6, 10, 12, 12, 10, 6, 0]);;
});

test("test solution 2", () => {
  expect(sol2.part2()).toBe(71503);
});
